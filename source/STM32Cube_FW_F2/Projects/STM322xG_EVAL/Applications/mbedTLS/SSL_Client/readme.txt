...
crypto library and LwIP TCP/IP stack on STM32F2 family.

Scenario 1:
===========
In this screnario the client (STM322xG_EVAL):
...
Scenario 2:
===========
In this second scenario, the the client (STM322xG_EVAL):
- connects to local network either through DHCP or static IP address.
- establishes a secured connect to a second STM322xG_EVAL borad running
...
@par Directory contents
    ...
    - mbedTLS/SSL_Client/Inc/lcd_log_conf.h                 config file for the LCD/LOG API
    ...
    - mbedTLS/SSL_Client/Src/stm32f2xx_hal_msp.c            HAL MSP module
    - mbedTLS/SSL_Client/Src/stm32f2xx_it.c                 Interrupt handlers
    ...
    - mbedTLS/SSL_Client/Src/system_stm32f2xx.c             STM32 system clock configuration file

@note Because of the low RAM size on the ST32F2 family the mbedtls_config.h is not the full-featured configuration.It is instead based on the configuration provided by the "config-suite-b.h" under mbedTLS-2.4.0/configs.
...
@par Hardware and Software environment
  ...
  - This application runs on STM32F207G/STM32F217G Devices.
  ...
  - This application has been tested with two scenarios.
     ...
     - using two STM32F2xG_EVAL boards the first as server, the second as client.
  ...
  - Senario 2:
      - a second STM322xG_EVAL running the "mdedTLS/SSL_Server" application

  - STM322xG_EVAL setup
    - Connect STM322xG_EVAL to your local network (through a straight ethernet cable).
